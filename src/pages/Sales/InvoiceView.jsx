...
export const InvoiceView = () => {
  ...
  if (invoice?.items) {
    ...
    while (itemsAdded < invoice.items.length) {
      let capacity;
      const remaining = invoice.items.length - itemsAdded;
      ...
      const chunk = invoice.items.slice(itemsAdded, itemsAdded + capacity);
    ...
    if (requiresEmptyFooterPage) {
      pdfChunks.push([]); // Add dedicated footer page
    }
  }
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        // Wait briefly for layout and fonts to fully resolve
        ...
        // Calculate dynamic height to maintain exact aspect ratio - prevents any visual distortion!
        let imgHeight = (imgProps.height * pdfWidth) / imgProps.width;
        let imgWidth = pdfWidth;
        const pdfPageHeight = 297; // A4 height in mm
        
        // FIT TO PAGE LOGIC: If the rendered chunk is slightly taller than A4, scale it down proportionally to fit the height!
        ...
        const xOffset = (pdfWidth - imgWidth) / 2; // Center horizontally if scaled down
  ...
  return (
    <div className="space-y-4 max-w-[794px] mx-auto pb-8 print:w-[794px] print:max-w-[794px] print:mx-auto print:pb-0 print:space-y-0">
      ...
      {/* 
        =======================================================================
        MAIN VISIBLE INVOICE CONTAINER (USED FOR SCREEN & NATIVE BROWSER PRINT)
        =======================================================================
      */}
      <div 
        id="invoice-content"
      ...
      >
        ...
        {/* Addresses */}
        <div className="pb-4 print:pb-2 print:break-inside-avoid z-10 relative">
          <div className="text-xs">
            <h3 className="font-bold text-sm mb-2 border-b border-gray-500 pb-1 w-1/2">Billed To</h3>
            {invoice.customerId ? (
          ...
          <div className="min-h-[400px]">
            <table className="w-full border-collapse border border-gray-400 text-xs text-left">
              <thead className="bg-gray-50">
                <tr>
                  <th className="border border-gray-400 p-1.5 font-bold text-center w-12">No.</th>
              ...
              <tbody>
                {paginatedItems.map((item, i) => (
                  <tr key={i} className="print:break-inside-avoid">
                    <td className="border border-gray-400 p-1.5 text-center">{(currentPage - 1) * itemsPerPage + i + 1}</td>
                    ...
                    <td className="border border-gray-400 p-1.5 text-right">{item.sellingPrice.toFixed(2)}</td>
                ...
                {paginatedItems.length === 0 && (
                  <tr>
                    <td colSpan={6} className="border border-gray-400 p-4 text-center text-gray-500">No items found.</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <table className="w-full border-collapse border border-gray-400 text-xs text-left">
            ...
            <tbody>
              <tr>
                ...
                <td className="border border-gray-400 p-1.5 text-right">Rs. {invoice.subTotal.toFixed(2)}</td>
              </tr>
              <tr>
                <td className="border border-gray-400 p-1.5">Total GST</td>
                <td className="border border-gray-400 p-1.5 text-right">Rs. {invoice.totalGst.toFixed(2)}</td>
              </tr>
              {invoice.roundOff !== 0 && (
                <tr>
                  <td className="border border-gray-400 p-1.5">Round Off</td>
                  <td className="border border-gray-400 p-1.5 text-right">Rs. {invoice.roundOff.toFixed(2)}</td>
                </tr>
              )}
              <tr>
                <td className="border border-gray-400 p-1.5 bg-gray-50 font-bold">Grand Total</td>
                <td className="border border-gray-400 p-1.5 bg-gray-50 text-right font-bold">Rs. {invoice.grandTotal.toFixed(2)}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div className="print:break-inside-avoid">
          <div className="mt-4 flex flex-row justify-between gap-8 z-10 relative">
            ...
            <div className="flex-1 space-y-4">
              <div className="print:break-inside-avoid">
                ...
                <table className="w-full border-collapse border border-gray-400 text-xs text-left">
                  <tbody>
                    <tr>
                      <td className="border border-gray-400 p-1 font-bold bg-gray-50 w-32 text-gray-600">Account Holder</td>
                      <td className="border border-gray-400 p-1">{settings?.bankDetails?.accountName || settings?.name || "YOUR COMPANY PRIVATE LIMITED"}</td>
                    </tr>
                    <tr>
                      <td className="border border-gray-400 p-1 font-bold bg-gray-50 text-gray-600">Account Number</td>
                      <td className="border border-gray-400 p-1">{settings?.bankDetails?.accountNumber || "50200116941777"}</td>
                    </tr>
                    <tr>
                      <td className="border border-gray-400 p-1 font-bold bg-gray-50 text-gray-600">IFSC Code</td>
                      <td className="border border-gray-400 p-1">{settings?.bankDetails?.ifscCode || "HDFC0002321"}</td>
                    </tr>
                    {settings?.bankDetails?.bankName && (
                      <tr>
                        <td className="border border-gray-400 p-1 font-bold bg-gray-50 text-gray-600">Bank Name</td>
                        <td className="border border-gray-400 p-1">{settings.bankDetails.bankName} {settings.bankDetails.branch ? `(${settings.bankDetails.branch})` : ''}</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            {/* Right Column: Paid Stamp & Signatures */}
            <div className="w-[340px] flex flex-col justify-between items-end relative pt-2">
              ...
              <div className="w-full flex flex-row justify-between gap-6 mt-24">
          ...
          >
            ...
            {pageIndex === 0 && (
              <>
                ...
                <div className="pb-1 z-10 relative">
                  <div className="text-xs">
                    <h3 className="font-bold text-sm mb-1 border-b border-gray-500 pb-1 w-1/2">Billed To</h3>
                    {invoice.customerId ? (
                      <div className="space-y-0.5">
                        <p className="font-bold text-sm">{invoice.customerId.name}</p>
                        <p className="whitespace-pre-wrap leading-relaxed max-w-sm">{invoice.customerId.address || 'Address not provided'}</p>
            ...
            {chunk.length > 0 && (
              <div className="mt-2 z-10 relative">
                ...
                <thead className="bg-gray-50">
                  <tr>
                    <th className="border border-gray-400 py-1 px-1.5 font-bold text-center w-12">No.</th>
                ...
                <tbody>
                  {chunk.map((item, i) => {
                    // Calculate absolute index safely based on previous chunks
                    const absoluteIndex = pdfChunks.slice(0, pageIndex).reduce((acc, c) => acc + c.length, 0) + i + 1;
                    return (
                      <tr key={i}>
                        <td className="border border-gray-400 py-1 px-1.5 text-center">{absoluteIndex}</td>
                        ...
                        <td className="border border-gray-400 py-1 px-1.5 text-right">{item.sellingPrice.toFixed(2)}</td>
                  ...
                  <table className="w-full border-collapse border border-gray-400 text-xs text-left">
                    <tbody>
                      <tr>
                        ...
                        <td className="border border-gray-400 p-1.5 font-bold text-right bg-gray-50 w-40">Amount</td>
                      </tr>
                      <tr>
                        <td className="border border-gray-400 p-1.5">Subtotal</td>
                        <td className="border border-gray-400 p-1.5 text-right">Rs. {invoice.subTotal.toFixed(2)}</td>
                      ...
                      <tr>
                        <td className="border border-gray-400 p-1.5 font-bold">Grand Total</td>
                        <td className="border border-gray-400 p-1.5 text-right font-bold">Rs. {invoice.grandTotal.toFixed(2)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>

                {/* Bank Details & Terms & Signatures */}
                <div className="flex flex-row justify-between gap-8 mb-2">
                  {/* Left Column */}
                  ...
                  <div className="w-[340px] flex flex-col justify-between items-end relative pt-2">
                    ...
                    <div className="w-full flex flex-row justify-between gap-6 mt-20">
                      <div className="flex-1">
                        <div className="border-t border-gray-800 pt-1 text-center text-xs font-semibold">
                          Customer Signature
                        </div>
